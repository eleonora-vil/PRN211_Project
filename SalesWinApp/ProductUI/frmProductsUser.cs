...
namespace SalesWinApp
{
    public partial class frmProductsUser : Form
    {
        ...
        {
            InitializeComponent();
            this.Activated += new EventHandler(this.frmProductsUser_Activated);

        }
        private void frmProductsUser_Activated(object sender, EventArgs e)
        {
            // This event handler will be called whenever the form becomes active again.
            // You can put your refresh logic here.

            // For example, let's refresh the product list.
            var source = new BindingSource();
            source.DataSource = repository.GetProducts();
            LoadProductList(source);

            // You may also want to update other data that should be refreshed when the form becomes active.
        }
